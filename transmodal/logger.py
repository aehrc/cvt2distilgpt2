...
class ExperimentWriter(object):
        ...
        if os.path.isfile(self.metrics_file_path):
            ...
            while True:
                self.metrics_file_path = os.path.join(
                    self.log_dir, "metrics_" + str(count) + ".csv"
                )
                count += 1
    ...
    def log_metrics(
        ...
        if metrics["step"] not in self.metrics:
            self.metrics[metrics["step"]] = {}
        self.metrics[metrics["step"]].update(metrics)
        self.headers.update(metrics)
