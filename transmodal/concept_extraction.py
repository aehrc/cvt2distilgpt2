from quickumls_modified import QuickUMLS
import re
import spacy
...
class ConceptExtraction:
    ...
    It takes advantage of Simstring (Okazaki and Tsujii, 2010) for approximate string matching.
    Repository: https://github.com/Georgetown-IR-Lab/QuickUMLS
    """

    def __init__(self):
        self.install_dir = "/datasets/work/hb-mlaifsp-mm/source/Datasets/umls/2021AA_1.4.0.post1_14-06-21"
        self.ver = self.install_dir.rsplit("/", 1)[-1]
        self.accepted_semtypes = {
        ...
        self.matcher.nlp = spacy.load("en_core_web_sm")

    def __call__(self, medical_string):
        """
        Uses QuickUMLS to extract UMLS CUIs from a given string.

        Argument/s:
            medical_string - string with medical concepts.

        Returns:
            A list containing the CUIs
        """
        search = self.search(medical_string)
        if len(search) == 0:
            cuis = []
        else:
            cuis = list(set([i["cui"] for i in search[0]]))
        return cuis

    def search(self, medical_string):
        """
        QuickUMLS is used to extract medical concepts from the given string.

        Argument/s:
            medical_string - string with medical concepts.

        Returns:
            A list of dictionaries containing the extracted concepts.
        """
        if medical_string == None:
            return []
        medical_string = re.sub("[^A-Za-z0-9]+", " ", medical_string).strip()
        return self.matcher.match(medical_string, best_match=True, ignore_syntax=False)

    def age_to_cuis(self, age: int):
        """
        Converts a given age to a set of UMLS CUIs.

        Argument/s:
            age -  the age.

        Returns:
            cuis - the set of UMLS CUIs for the given age.
        """
        cuis = []

        if age >= 85:
            cuis.append("C0178694")
        if age >= 75:
            cuis.append("C4035139")
        if age >= 65:
            cuis.append("C3843802")
        if 70 <= age <= 74:
            cuis.append("C4035745")
        if 65 <= age <= 69:
            cuis.append("C4035746")
        if 60 <= age <= 64:
            cuis.append("C4035747")
        if 55 <= age <= 59:
            cuis.append("C4035748")
        if 50 <= age <= 54:
            cuis.append("C4035749")
        if 45 <= age <= 49:
            cuis.append("C4035750")
        if 40 <= age <= 44:
            cuis.append("C4035751")
        if 35 <= age <= 39:
            cuis.append("C4035752")
        if 30 <= age <= 34:
            cuis.append("C4035753")
        if 25 <= age <= 39:
            cuis.append("C4035754")
        if 20 <= age <= 24:
            cuis.append("C4035755")
        if 15 <= age <= 19:
            cuis.append("C4035756")

        if 19 <= age <= 44:
            cuis.append("C0001675")
        if 13 <= age <= 18:
            cuis.append("C0001578")
        if 6 <= age <= 12:
            cuis.append("C0008059")
        if 2 <= age <= 5:
            cuis.append("C0008100")

        if 40 <= age <= 64:
            cuis.append("C3843803")
        if 35 <= age <= 64:
            cuis.append("C0596727")
        if 19 <= age <= 64:
            cuis.append("C5425881")

        if 25 <= age <= 39:
            cuis.append("C3843804")
        if 15 <= age <= 24:
            cuis.append("C3843805")
        if 5 <= age <= 14:
            cuis.append("C3843806")

        if 21 <= age <= 34:
            cuis.append("C0597696")
        if 6 <= age <= 11:
            cuis.append("C0596947")
        if 2 <= age <= 11:
            cuis.append("C1707381")

        if 18 <= age <= 65:
            cuis.append("C1706730")

        if age < 5:
            cuis.append("C3843807")

        return cuis

    def projection_to_cui(self, projection: str):
        """
        Converts a given projection/view to a UMLS CUI.

        Argument/s:
            projection - the projection/view of the medical image.

        Returns:
            cui - the UMLS CUIs for the given projection/view.
        """
        if projection == "PA":
            cui = "C1996865"
        elif projection == "AP":
            cui = "C1999039"
        elif projection in ["Lateral", "L"]:
            cui = "C0442197"
        elif projection in ["RL"]:
            cui = "C0450415"
        elif projection in ["LL"]:
            cui = "C0450415"
        else:
            raise ValueError("{} is not a valid projection/view.".format(projection))
        return cui


if __name__ == "__main__":
    string = "The ulna has dislocated posteriorly from the trochlea of the humerus."
    concept_extractor = ConceptExtraction()
    print(concept_extractor.search(string))
